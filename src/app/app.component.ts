...
interface PresentationDraft { name: string; units: number; priceInput: string; }
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  addPresentation() { this.draft.presentations.push({name:'',units:1,priceInput:'0.00'}); }
  ...
  async save() {
    ...
    const product:Product={barcode:this.draft.barcode.trim(),name:this.draft.name.trim(),price:presentations[0].price,presentations,tags:this.draft.tags.filter(Boolean),image:this.draft.image,updatedAt:Date.now()};
  ...
  private empty(barcode=''):ProductDraft { return {barcode,name:'',tags:[],presentations:[{name:'Unidad',units:1,priceInput:'0.00'}]}; }
}
